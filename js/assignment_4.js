...
(function() {
  ...
  function makeScatterPlot(csvData) {
    ...
    mapFunc = drawAxes(axesLimits, "fertility_rate", "life_expectancy");
  ...
  function makeDropDown() {
    ...
    // find time data
    let dataDD = Array.from(new Set(data.map(year => year["time"])))
    .map(time => {
      return {
        time : data.find(year => year["time"] == time)["time"]
      }
    })
    ...
    dropDown.on("change", function () {
          ...
          time = selected;
          var data_by_year = data.filter(s => s["time"] == selected)
          svgContainer.selectAll("circle").remove()
          plotData(mapFunc, data_by_year);
    });
  }
  ...
  function plotData(map, thedata) {
    ...
    let pop_data = thedata.map((row) => + row["pop_mlns"]);
    ...
    var data_by_year = data.filter(data => data.time === time);
